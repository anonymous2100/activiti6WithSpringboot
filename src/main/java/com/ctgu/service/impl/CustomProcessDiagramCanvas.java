...
public class CustomProcessDiagramCanvas
{
	 ...
	 * side of the canvas. This is useful for diagrams that have white space there. Everything beneath these minimum
	 * values will be cropped. It's also possible to pass a specific font name and a class loader for the icon images.
	 */
	...
	private static Point getEllipseIntersection(Shape shape, Line2D.Double line)
	{
		...
		double x = shape.getBounds2D()
				.getWidth() / 2 * Math.cos(angle) + shape.getBounds2D()
						.getCenterX();
		double y = shape.getBounds2D()
				.getHeight() / 2 * Math.sin(angle) + shape.getBounds2D()
						.getCenterY();
		Point p = new Point();
		p.setLocation(x, y);
		return p;
