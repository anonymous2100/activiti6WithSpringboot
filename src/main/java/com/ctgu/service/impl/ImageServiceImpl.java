...
public class ImageServiceImpl implements ImageService
{
		...
		for (HistoricActivityInstance currentActivityInstance : finishedActivityInstances)
		{
			...
			else
			{
				...
				for (SequenceFlow sequenceFlow : sequenceFlows)
				{
					for (HistoricActivityInstance historicActivityInstance : historicActivityInstances)
					{
								...
								.equals(sequenceFlow.getTargetRef()))
						{
							...
							map.put("highLightedFlowStartTime", historicActivityInstance.getStartTime()
									.getTime());
							tempMapList.add(map);
						}
					}
				...
				if (tempMapList != null && tempMapList.size() > 0)
				{
					...
					{
						long highLightedFlowStartTime = Long.valueOf(map.get("highLightedFlowStartTime")
								.toString());
	 ...
	 * @return
	 */
	public List<String> getMyHighLightedFlows(BpmnModel bpmnModel,
		...
		for (int k = 0; k < finishedActivityInstanceList.size(); k++)
		{
			...
			else
			{
				...
				for (int i = 0; i < historicActivityInstanceList.size(); i++)
				{
					...
					if (i >= k && historicActivityInstance.getActivityId()
	...
	public byte[] getFlowImgByProcInstId(String procInstId) throws Exception
	{
		...
		try
		{
			...
			for (HistoricActivityInstance historicActivityInstance : historicActivityInstanceList)
			{
				...
				log.info("已执行的节点[{}-{}-{}-{}]", historicActivityInstance.getId(),
			...
			// 获取已流经的流程线，需要高亮显示高亮流程已发生流转的线id集合
			List<String> highLightedFlowIds = getMyHighLightedFlows(bpmnModel, historicActivityInstanceList);

			// 使用默认配置获得流程图表生成器，并生成追踪图片字符流
			imageStream = ((CustomProcessDiagramGenerator) processDiagramGenerator).generateDiagramCustom(bpmnModel,
					"png", highLightedActivitiIdList, runningActivitiIdList, highLightedFlowIds, "宋体", "宋体", "宋体", null,
					2.0);
