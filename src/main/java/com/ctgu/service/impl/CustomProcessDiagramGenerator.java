...
/**
 * Class to generate an image based the diagram interchange information in a BPMN 2.0 process. 用于在BPMN
 * 2.0流程中基于图交换信息生成图像的类。 <br>
 * 备注：CustomProcessDiagramCanvas.java和CustomProcessDiagramGenerator.java都是用来生成并高亮流程定义图片的类
 */
public class CustomProcessDiagramGenerator implements ProcessDiagramGenerator
{
			...
			String activityFontName, String labelFontName, ClassLoader customClassLoader, double scaleFactor)
	{
		 ...
		 */
		for (FlowNode flowNode : bpmnModel.getProcesses()
				.get(0)
	 ...
	 * @author Fuxs
	 */
	private static void drawRunningActivitiHighLight(CustomProcessDiagramCanvas processDiagramCanvas,
			GraphicInfo graphicInfo)
	{
		processDiagramCanvas.drawRunningActivitiHighLight((int) graphicInfo.getX(), (int) graphicInfo.getY(),
				(int) graphicInfo.getWidth(), (int) graphicInfo.getHeight());
